...
class SollyaFixedConverter:
    ...
    def node_format_repr(self, node_format):
        return str(node_format).replace(".","p").replace("-","m")

    def tag_new_op(self, poly, node_format, op_list, desc="UNK"):
        if self.lutetia_tag_mode:
            ...
            tag = "{}_{}_{}{}".format(
                out_tag,
                self.node_format_repr(node_format.fixed_format),
                desc,
                ("_" if len(op_list) else "") + "_".join(self.get_name(op) for op in op_list)
            )
            self.name_map[poly] = out_tag
            return tag

    def tag_new_node_legacy(self, node_ml_format, op_list, desc="UNK"):
        out_tag = self.get_new_letter()
        tag = "{}_{}_{}{}".format(
            out_tag,
            self.node_format_repr(node_ml_format),
            desc,
            ("_" if len(op_list) else "") + "_".join(self.get_name(op) for op in op_list)
        )
        return tag
    ...
    def parse_sollya_addition(self, poly):
        ...
        tag = self.tag_new_op(poly, ml_format, [poly.struct.q, poly.struct.r], "Add")
    ...
    def parse_sollya_multiplication(self, poly):
        ...
        lhs = self.parse_sollya_poly(poly.struct.q)
        rhs = self.parse_sollya_poly(poly.struct.r)
        tag = self.tag_new_op(poly, ml_format, [poly.struct.q, poly.struct.r], "Mul")
    ...
    def parse_sollya_constant(self, cst):
        ...
        result = full_cst
        tag = self.tag_new_op(cst, ml_format, [], "Cst")
    ...
    def parse_sollya_variable(self, var):
        ...
        if ml_format.fixed_format is input_precision:
            ...
            conv_node = Conversion(
                self.root_var,
                precision=ml_format.fixed_format,
                tag=var.struct.name or self.tag_new_node_legacy(ml_format.fixed_format, [self.root_var], "Conv")
            )
            result = expand_node(self. conv_node)
            # only tag converted variabe as we do not want to modify
            # the tag associated to an input Variable
            tag = self.tag_new_op(result, ml_format, [], "Var")
        return tag_node(tag, result)


    def get_power(self, k, ml_format):
        epsilon = ml_format.abs_error
        precision = ml_format.fixed_format
        result = None
        if k == 1:
            if precision is self.root_var.precision:
                ...
                result = Conversion(self.root_var, precision=precision, abs_error=epsilon, tag="Conv")
            ...
            if result_format != precision:
                result = Conversion(
                    ...
                    tag=self.tag_new_node_legacy(power_format, [result], "Conv")
                )
        else:
    ...
    def parse_sollya_power(self, power):
        ...
        if self.lutetia_tag_mode:
            ...
            tag = "pow{}_{}".format(k, self.node_format_repr(ml_format.fixed_format))
    ...
    def build_powers_for(self, tree):
        ...
        if k == 1:
            if precision is self.root_var.precision:
                ...
                result = Conversion(self.root_var, precision=precision, tag=self.tag_new_node_legacy(power_format, [result], "Conv"))
        elif k > 1:
            if k in self.power_table:
                for power_format, power_node, power_epsilon in self.power_table[k]:
                    if power_format == precision and power_epsilon <= ml_format.abs_error:
                        result = power_node
                        exist = True
                    elif power_epsilon <= ml_format.abs_error:
                        result = Conversion(
            ...
            if result == None:
                ...
                result = Multiplication(r1, r2, precision=ml_format.fixed_format, tag="MulPow_%d" % k)
        else:

...
def expand_node(node):
    if node.precision in [ML_Binary32, ML_Binary64]:
        return (node,)
    elif node.precision in [ML_DoubleDouble]:
        return (node.hi, node.lo)
    elif node.precision in [ML_TripleDouble]:
        return (node.hi, node.me, node.lo)
    else:
        raise NotImplementedError

def tag_tuple(tag, expanded_tuple, **extra_attributes):
    ...
    for op, suffix in zip(expanded_tuple, suffix[len(expanded_tuple)]):
        op.set_tag("{}{}".format(tag, suffix))
        op.set_attributes(**extra_attributes)
    return expanded_tuple

def tag_tuple_from_poly(poly, expanded_tuple, **extra_attributes):
    tag = poly.struct.name
    return tag_tuple(tag, expanded_tuple, **extra_attributes)


class SollyaConverter:
    ...
    def parse_sollya_addition(self, poly):
        ...
        #precision, accuracy = self.parse_sollya_format(poly.struct.format)
        ...
        #return Addition(lhs, rhs, precision=precision, rel_error=accuracy)
        #return tag_tuple_from_poly(poly, ml_format.meta_block.expand(lhs, rhs))
        tag = self.tag_new_op(poly, [poly.struct.q, poly.struct.r], "Add_{}".format(ml_format.meta_block.__class__.__name__))
        return tag_tuple(tag, ml_format.meta_block.expand(lhs, rhs), debug=debug_multi)

    def parse_sollya_multiplication(self, poly):
        """ generate a Metalibm Multiplication node """
        ml_format = self.parse_sollya_format(poly.struct.format) # precision, accuracy = self.parse_sollya_format(poly.struct.format)
        lhs = self.parse_sollya_poly(poly.struct.q)
        rhs = self.parse_sollya_poly(poly.struct.r)
        tag = self.tag_new_op(poly, [poly.struct.q, poly.struct.r], "Mul_{}".format(ml_format.meta_block.__class__.__name__))
        #return tag_tuple_from_poly(poly, ml_format.meta_block.expand(lhs, rhs))
        return tag_tuple(tag, ml_format.meta_block.expand(lhs, rhs), debug=debug_multi)
    ...
    def parse_sollya_constant(self, cst):
        ...
        value = cst.struct.c
        #precision, accuracy = self.parse_sollya_format(cst.struct.format)
        ml_format = self.parse_sollya_format(cst.struct.format)
        # return Constant(value, precision=precision, rel_error=accuracy)
        full_cst = Constant(value, precision=ml_format.mp_node.precision)
        if ml_format.mp_node.precision in [ML_Binary32, ML_Binary64]:
            result = (full_cst,)
        elif ml_format.mp_node.precision is ML_DoubleDouble:
            result = full_cst.hi, full_cst.lo
        elif ml_format.mp_node.precision is ML_TripleDouble:
            result = full_cst.hi, full_cst.me, full_cst.lo
        else:
            raise Exception("unsupport ml_format.mp_node.precision in parse_sollya_constant")
        tag = self.tag_new_op(cst, [], "Cst")
        #return tag_tuple_from_poly(cst, result)
        return tag_tuple(tag, result, debug=debug_multi)
    ...
    def parse_sollya_variable(self, var):
        ...
        input_precision = self.parse_sollya_format(var.struct.inputformat).mp_node.precision
        if ml_format.mp_node.precision is input_precision:
            result = expand_node(self.root_var)
        else:
            result = expand_node(Conversion(
                self.root_var,
                precision=ml_format.mp_node.precision,
                tag=var.struct.name
            ))
        tag = self.tag_new_op(var, [], "Var")
        return tag_tuple(tag, result)


    def get_power(self, k, ml_format):
        epsilon = ml_format.mp_node.epsilon
        precision = ml_format.mp_node.precision
        result = None
        if k == 1:
            if precision is self.root_var.precision:
                ...
                result = expand_node(Conversion(self.root_var, precision=precision, rel_error=epsilon))
            ...
            if len(possible_nodes) == 0:
                for power_format, power_node, power_epsilon in self.power_table[k]:
                    if power_epsilon <= epsilon:
                        if precision == ML_Binary64 and power_format in [ML_DoubleDouble, ML_TripleDouble] or \
                           precision == ML_DoubleDouble and power_format in [ML_TripleDouble]:
                            possible_nodes.append((power_format, power_node, power_epsilon))
            ...
            if result_format != precision:
                result = expand_node(
                    Conversion(
                        BuildFromComponent(*result, precision=power_format),
                        precision=precision)
                )
        else:
    ...
    def parse_sollya_power(self, power):
        ...
        # if the k-th power of root_var has already been computed
        # in a format equal or greater to the requested format we use
        # it for this power

        if self.lutetia_tag_mode:
            return tag_tuple_from_poly(power, self.get_power(k, ml_format))
        else:
            tag = "pow{}".format(k)
            self.name_map[power] = tag
            return tag_tuple(tag, self.get_power(k, ml_format), debug=debug_multi)
    ...
    def build_powers_for(self, tree):
        ...
        if k == 1:
            if precision is self.root_var.precision:
                ...
                result = expand_node(Conversion(self.root_var, precision=precision))
        elif k > 1:
            if k in self.power_table:
                for power_format, power_node, power_epsilon in self.power_table[k]:
                    if power_format == precision and power_epsilon <= ml_format.mp_node.epsilon:
                        result = power_node
                        exist = True
                    elif power_epsilon <= ml_format.mp_node.epsilon:
                        result = expand_node(
                            Conversion(
                                BuildFromComponent(*power_node, precision=power_format),
                                precision=precision)
                            )
            if result == None:
                r1 = self.build_powers_for(tree.struct.q)
                r2 = self.build_powers_for(tree.struct.r)
                result = ml_format.meta_block.expand(r1, r2)
                #result = Multiplication(r1, r2, precision=precision, rel_error=accuracy)
        else:
